...
const TOAST_LIMIT = 1
// Time before the toast is automatically dismissed (ms)
const TOAST_REMOVE_DELAY = 5000
// Small delay to allow close animation before removing from DOM (ms)
...
type ToasterToast = ToastProps & {
  ...
  // Si true, este toast no se eliminará automáticamente (persistente) hasta que se llame a dismiss()
  persist?: boolean
...
export const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case "REMOVE_TOAST":
      ...
      // Limpieza de timeouts asociados
...
function toast({ ...props }: Toast) {
  ...
  // Programar auto-dismiss a menos que se solicite persistencia
  if (!props.persist) {
