...
async function crearFuncionSQL() {
    console.log('🔧 CREANDO FUNCIÓN crear_embarque_normalizado...\n');
...
BEGIN
    ...
    IF p_tipo_servicio_id IS NOT NULL THEN
        SELECT 
            COALESCE(ts.precio_flete, 0),
            COALESCE(ts.precio_operador, 0),
            ts.nombre
    ...
    INSERT INTO embarques_nuevo (
        ...
        p_contenido, p_peso, p_load_number, now(), now()
    );
...
$$ LANGUAGE plpgsql;
    ...
    try {
        // Intentar ejecutar la función usando query directo
        const { error } = await supabase.rpc('sql', { query: funcionSQL });
